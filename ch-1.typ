...
#figure(
  image("images/ch-1/1-universe_composition.jpg", width: 100%),
...
Con l'arrivo di nuovi dati, sono stati sviluppati nuovi modelli cosmologici con diversi valori di abbondanza (per *abbondanza* $Omega$ si intende una quantità in riferimento a un valore standard).
#figure(
  image("images/ch-1/2-cosmic_triangle_measures.png", width: 80%),
...
$Lambda$#sub[CDM] è il modello attualmente più accreditato ed è quello che viene detto un *modello concordatario* _(concordance model)_, ovvero che sia in accordo con i dati provenienti dalle varie misure.
...
Essendo un processo a soglia, l'energia rilasciata è sempre la stessa.
Dunque questo tipo di eventi può essere utilizzato come *candela standard* (una candela standard è un processo di cui si conosce la luminosità alla sorgente e di cui si può ricavare la distanza confrontando la luminosità con il flusso osservato).\
#let fig_sne_dist_redshift = figure(
  image("images/ch-1/3-supernovae_distance.jpeg", width: 90%),
...
#wrap-content(fig_sne_dist_redshift, align:right)[
  ...
  Questo ci permette di ridurre lo spazio dei parametri in cui il modello potrà ricadere.
]
]
/ Cosmic Microwave Background (CMB):#[\
Il fondo cosmico a microonde consiste in una radiazione omogenea e isotropa proveniente da un'emissione di corpo nero risalente al periodo in cui la radiazione si è disaccoppiata dalla materia.
#figure(
  image("images/ch-1/4-cmb_spectrum.png", width: 70%),
...
La temperatura corrispondente è $T = qty("2.725", "K")$ in tutte le direzioni, con fluttuazioni di $delta T approx qty("e-5", "K")$
(possiamo confrontarle con le fluttuazioni del raggio terrestre che arrivano fino a $approx 0.33%$).\
Le fluttuazioni sono state osservate con sempre miglior risoluzione da diversi esperimenti
#figure(
  image("images/ch-1/5-cmb_anisotropies.jpg", width: 60%),
...
Questa teoria fu confutata per la prima volta nel 1989 con la pubblicazione della prima _Galaxy Survey_ da parte di Margaret Geller e John Huchra.
...
#subpar.grid(
  figure(
    image("images/ch-1/6-1-galaxy_survey_geller.png"),
    caption: [Geller-Huchra Galaxy Survey]
  ),
  figure(
    image("images/ch-1/6-2-galaxy_survey_desi.jpg"),
...
Nelle prime fasi la materia era _accoppiata_ alla radiazione (principalmente attraverso scattering), dunque era soggetta alla forza gravitazionale e alla pressione di radiazione.
...
#figure(
  image("images/ch-1/7-bao_diagram.jpg", width: 60%),
...
#figure(
  image("images/ch-1/8-bao_bump.jpg", width: 60%),
