...
#aeq[$
  tensor(Tilde(g), -mu, -nu) &= pdv(Tilde(x)^alpha, x^mu) pdv(Tilde(x)^beta, x^nu) tensor(g, -alpha, -beta)\
...
#aeq[$
  cal(H) = dv(ln(a), eta) --> dv(ln[a(1+Phi)], eta) = 1/(a (1+Phi)) [a' (1+Phi) + a Phi'] = cal(H) + Phi'/(1+ Phi)
$]
...
#aeq[$
  -4 cal(H) overline(rho) (1 + w) va(v) - overline(rho) (1 + w) grad Psi - c_s^2 grad delta rho - grad^TT dot vt(Pi)
$]
...
$
  cases(
    Phi'' + 1/3 laplacian (Psi - Phi) + cal(H) Psi' + 2 cal(H) Phi' + (2 cal(H)' + cal(H)^2) Psi = 4 pi G a^2 delta p,
...
#aeq[$
  cal(H)^2 = (8 pi G)/3 a^2 underbracket(overline(rho), prop a^(-3)) ~ a^(-1) quad ==> quad cal(H) ~ a^(-1/2) ~ 1/(2 eta)
$]
...
- #[
    ...
    $
      zeta(a_"RD") = zeta(a_"MD") quad & ==> quad 6/4 Psi(a_"RD") = 5/3 Psi(a_"MD") \
                                       & ==> quad heq(Psi(a_"MD") = 9/10 Psi(a_"RD"))
    $
...
#figure(
  image("images/ch-7/1-curvature_evolution.png", width: 80%),
)
...
$
  integral dd(s, 3) xi(s) & = integral dd(s, 3) 1/V integral dd(x, 3) Delta_m (va(x)) Delta_m (va(x) + va(s)) \
                          & = 1/V integral dd(x_1, 3) integral dd(x_2, 3) Delta_m (va(x)_1) Delta_m (va(x)_2) \
                          & = 1/V integral dd(x_1, 3) Delta_m (va(x)_1) integral dd(x_2, 3) Delta_m (va(x)_2) = 0
$
...
con
#aeq[$
  T(k) = cases(1 wide &k<k_"eq", k^(-2) ln(k/k_"eq") &k>k_"eq") wide "e" wide cal(P)_(zeta zeta) = A_s (k/k_p)^(n_s - 1)
$]
...
#aeq[$
  M(va(x), <R) &= integral_(V_R) dd(y, 3) med rho(y) = integral dd(y, 3) med rho(va(y)) med Theta(1- abs(va(x)- va(y))/R)\
...
$
  W_"TH" (va(k), R) & = integral dd(y, 3) med e^(-i va(k) dot va(y)) med W_"TH" (abs(va(y)), R) \
                    & = 3/(k R)^3 [sin(k R) - k R cos(k R)] = W_"TH" (k, R)
$
...
#aeq[$
  n_h &= overline(n)_h + evaluated(dv(n, delta_l))_(delta_l = 0) delta_l + Order(delta_l^2)\
...
ma
#aeq[$
  pdv(ln(n_h), Tilde(overline(rho))) = overline(rho)/Tilde(overline(rho)) wide "e" wide pdv(ln(n_h), Tilde(delta)_l) = 1/(delta_c - delta_l) - 2 (delta_c - delta_l)/sigma_M^2
$]
quindi
